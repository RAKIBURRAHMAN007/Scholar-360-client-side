...
const AllScholarShip = () => {
    ...
    const filteredScholarships = allScholarships.filter((scholarship) => {
        ...
        return (
            (scholarshipName && scholarshipName.toLowerCase().includes(searchTerm.toLowerCase())) ||
            (universityName && universityName.toLowerCase().includes(searchTerm.toLowerCase())) ||
            (degreeName && degreeName.toLowerCase().includes(searchTerm.toLowerCase()))
        );
    });
    ...
    for (let i = 1; i <= totalPages; i++) {
        pageButtons.push(
            <button
                key={i}
                className={`px-4 py-2 rounded-md ${currentPage === i
    ...
    return (
        <div className="w-11/12 mx-auto mt-28 mb-20">
            ...
            <p className="text-center mb-10">
                "Discover prestigious scholarships from top universities around the globe. Explore
                opportunities to achieve <br className="md:block hidden" /> your academic dreams with funding
