  ...
  const handleCancel = (id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        axiosSecure.delete(`/allScholarship/${id}`).then((res) => {
                ...
                {Scholarships.map((scholarship) => (
                  <tr key={scholarship._id}>
                    ...
                    <td className="flex flex-col gap-2">
                      <Link
                        className="bg-blue-400 rounded-md btn-ghost btn-sm"
                        to={`/scholarshipDetails/${scholarship._id}`}
                      >
                        <button className="flex gap-1 justify-center py-1 items-center  ">
                          <FaEye /> Details
                        </button>
                      </Link>
                      <button
      ...
      {isModalOpen && selectedScholarship && (
        <div className="fixed inset-0 z-10 p-9 flex md:items-center mt-10 justify-center ">
          <div className="bg-white overflow-y-auto max-h-[90vh] p-6 rounded-lg shadow-lg w-96">
            ...
            <form onSubmit={handleUpdateScholarship} className="card-body">
